...
- **flex-grow：定义元素在主轴方向上的弹性大小，即默认情况下，元素会根据剩余空间的大小自动增长或缩小。例如，如果一个元素的 flex-grow 值为 2，则它会在水平方向上占据两个剩余空间。**
- **flex-shrink：定义元素在主轴方向上的弹性大小，即默认情况下，元素会根据剩余空间的大小自动收缩或扩大。例如，如果一个元素的 flex-shrink 值为 1，则当空间不足时，它会自动缩小。**
