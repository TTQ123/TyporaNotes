...
![image-20231229151741735](https://ttqblogimg.oss-cn-beijing.aliyuncs.com/image-20231229151741735.png)
...
**is和weher的区别在于where的优先级为0，is的级别更高。 is会找条件中优先级最高的选择器，在这个例子中和前面正常写法的优先级一样，但是后面书写所以会生效为蓝色**
...
**在has选择器没出来以前,子元素都是无法操作父元素的**

**1.通过子元素可以选择父元素  包含span的div样式会被设置**

这里相当于通过span选择了父元素div给其加上背景颜色
...
![image-20231208084554079](https://ttqblogimg.oss-cn-beijing.aliyuncs.com/image-20231208084554079.png)

![image-20231208084623429](https://ttqblogimg.oss-cn-beijing.aliyuncs.com/image-20231208084623429.png)

![image-20231208084728813](https://ttqblogimg.oss-cn-beijing.aliyuncs.com/image-20231208084728813.png)



**下面页面演示场景为一个大盒子套了六个小盒子**
...
![image-20231208084940477](https://ttqblogimg.oss-cn-beijing.aliyuncs.com/image-20231208084940477.png)

![image-20231208085025941](https://ttqblogimg.oss-cn-beijing.aliyuncs.com/image-20231208085025941.png)

![image-20231208085100554](https://ttqblogimg.oss-cn-beijing.aliyuncs.com/image-20231208085100554.png)



### 4.column行间距 row列间距 gap统一设置
...
![image-20231208085145092](https://ttqblogimg.oss-cn-beijing.aliyuncs.com/image-20231208085145092.png)
...
如果行轨道和列轨道的长度小于轴(子元素内容没有达到撑开整个grid布局)可以对轨道进行对齐
...
<view class="container">
    <view wx:for="{{list}}">
        <image src="{{item.src1}}" mode="aspectFill" />
        <view>
            <view class="text">
                <text>{{item.text}}</text>
            </view>
            <view class="vimg">
                <image src="../../image/爱心.png" mode="aspectFill" />
                <image src="../../image/收藏.png" mode="aspectFill" />
                <image src="../../image/话题.png" mode="aspectFill" />
            </view>
        </view>
    </view>
</view>
...
Page({
  data: {
    list:[
        {text:'部落冲突', src1: '../../image/blct.jpg'},
        {text:'保卫萝卜', src1: '../../image/bwlb.jpg'},
        {text:'蛋仔派对', src1: '../../image/dzpd.jpg'},
        {text:'和平精英', src1: '../../image/hpjy.jpg'},
        {text:'贪吃蛇', src1: '../../image/tcs.jpg'},
        {text:'金铲铲', src1: '../../image/jcc.jpg'},
        {text:'洗澡小子', src1: '../../image/qqdzz.png'},
        {text:'王者荣耀', src1: '../../image/wzry.jpg'},
        {text:'咸鱼翻身', src1: '../../image/xyzw.png'}
    ]
  }
})
```

![image-20231213205921228](https://ttqblogimg.oss-cn-beijing.aliyuncs.com/image-20231213205921228.png)

### 9.grid-row 和 grid-column
...
```css
.box1{
	// 这样即可水平垂直居中  在弹性项中设置margin表示吃掉剩余空间
	display:flex;
}
...
这种布局只需要给第三个元素设置margin:auto;

![image-20231125162624021](https://ttqblogimg.oss-cn-beijing.aliyuncs.com/image-20231125162624021.png)



这种布局

![image-20231125163211117](https://ttqblogimg.oss-cn-beijing.aliyuncs.com/image-20231125163211117.png)
...
<head>
    ...
    <style>
        ...
        .item{
            /* 七个 在不同平台就可以修改这个值 */
            --n:7; 
            /* 间隔 父元素宽度减去子元素宽度(所有子元素) 得到剩余空间 剩余空间除以14得到左右margin */
            --grp:calc((100% - 50px * var(--n)) / var(--n) /2);
            margin:10px var(--grp);
        }
    </style>
</head>
...
**:class{box:true,active:onActive}**

![image-20231127215544387](https://ttqblogimg.oss-cn-beijing.aliyuncs.com/image-20231127215544387.png)
...
gird无法解决dom结构的问题 但是可以进行常规的三列布局

![image-20240228163037655](https://ttqblogimg.oss-cn-beijing.aliyuncs.com/image-20240228163037655.png)



calc计算函数也可以实现

![image-20240228163104138](https://ttqblogimg.oss-cn-beijing.aliyuncs.com/image-20240228163104138.png)
